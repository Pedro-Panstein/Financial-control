...
export function DataImport({
  ...
  const validateTransaction = (
    ...
    if (isNaN(date.getTime())) {
      return {
        valid: false,
        error: `Linha ${lineNumber}: Data inválida "${dateStr}"`,
      };
    }

    // Validar tipo
    const type =
      typeStr.toLowerCase() === "ganho"
        ? "income"
        : typeStr.toLowerCase() === "gasto"
        ? "expense"
        : null;
    if (!type) {
      return {
        valid: false,
        error: `Linha ${lineNumber}: Tipo inválido "${typeStr}" (deve ser "Ganho" ou "Gasto")`,
      };
    }
    ...
    if (isNaN(amount) || amount <= 0) {
      return {
        valid: false,
        error: `Linha ${lineNumber}: Valor inválido "${amountStr}"`,
      };
    }
    ...
    if (!description.trim()) {
      return {
        valid: false,
        error: `Linha ${lineNumber}: Descrição é obrigatória`,
      };
    }
    ...
    if (!category.trim()) {
      return {
        valid: false,
        error: `Linha ${lineNumber}: Categoria é obrigatória`,
      };
    }
          ...
          <Alert>
            ...
            <AlertDescription>
              <div className="space-y-2">
                <p>
                  <strong>Formato esperado:</strong> Data, Descrição, Categoria,
                  Tipo, Valor
                </p>
                <p>
                ...
                <p>
                  <strong>Formato de valor:</strong> Use vírgula ou ponto para
                  decimais
                </p>
              </div>
            </AlertDescription>
          ...
          {file && (
            <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border">
              ...
              <p className="text-sm text-muted-foreground">
                Arquivo selecionado. Clique em "Processar" para analisar os
                dados.
              </p>
            </div>
          )}
              ...
              {result.imported.length > 0 && (
                <div className="space-y-2">
                  <h4 className="font-medium">
                    Preview das transações a serem importadas:
                  </h4>
              ...
              {result.errors.length > 0 && (
                <div className="space-y-2">
                  <Alert variant="destructive">
                    <AlertCircle className="h-4 w-4" />
                    <AlertDescription>
                      Foram encontrados {result.errors.length} erros no arquivo.
